...
XprVector<						\
  ...
  >							\
>							\
NAME(const Vector<T>& rhs) TVMET_CXX_ALWAYS_INLINE;
...
/*
 * unary_function(Vector<std::complex<T>>)
 */
...
XprVector<								\
  ...
  >									\
>									\
NAME(const Vector<std::complex<T>>& rhs) TVMET_CXX_ALWAYS_INLINE;
...
#endif
TVMET_IMPLEMENT_MACRO(isfinite)
#endif // defined(TVMET_HAVE_IEEE_MATH)
...
XprVector<								\
  ...
  >									\
>									\
NAME(const Vector<std::complex<T>>& rhs) {				\
  typedef XprUnOp<							\
      Fcnl_##NAME< std::complex<T> >,					\
      VectorConstReference<std::complex<T>>				\
    > 							expr_type;	\
